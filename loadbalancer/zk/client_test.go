...
func TestCreateParentNodes(t *testing.T) {
	...
	p, err := NewPublisher(c, "/validpath", newFactory(""), log.NewNopLogger())
	if err != stdzk.ErrNoServer {
		t.Errorf("unexpected error: %v", err)
	}
	if p != nil {
		t.Error("expected failed new Publisher")
	}

	p, err = NewPublisher(c, "invalidpath", newFactory(""), log.NewNopLogger())
